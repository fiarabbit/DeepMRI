...
class ThreeDimensionalAutoEncoder(chainer.Chain):
    ...
    def __init__(self):
        ...
        with self.init_scope():
            self.conv1 = L.ConvolutionND(3, 1, 16, (3, 3, 3), 2, 0)
            self.conv2 = L.ConvolutionND(3, 16, 32, (3, 4, 3), 2, 0)
            self.conv3 = L.ConvolutionND(3, 32, 64, (4, 4, 4), 2, 0)
            self.conv4 = L.ConvolutionND(3, 64, 128, (4, 4, 4), 2, 0)
            self.conv5 = L.ConvolutionND(3, 128, 256, (4, 5, 4), 1, 0)
            self.l1 = L.Linear(None, 100)
            self.bn1 = L.BatchNormalization(100)
            self.l2 = L.Linear(None, 20)
            self.bn2 = L.BatchNormalization(20)
            self.l3 = L.Linear(None, 100)
            self.bn3 = L.BatchNormalization(100)
            self.l4 = L.Linear(None, 256)
            self.bn4 = L.BatchNormalization(256)
            self.deconv5 = L.DeconvolutionND(3, 256, 128, (4, 5, 4), 1, 0)
            self.deconv4 = L.DeconvolutionND(3, 128, 64, (4, 4, 4), 2, 0)
            self.deconv3 = L.DeconvolutionND(3, 64, 32, (4, 4, 4), 2, 0)
            self.deconv2 = L.DeconvolutionND(3, 32, 16, (3, 4, 3), 2, 0)
            self.deconv1 = L.DeconvolutionND(3, 16, 1, (3, 3, 3), 2, 0)
    ...
    def calc(self, x):
        ...
        c5 = F.tanh(self.conv5(c4))
        l1 = F.tanh(self.bn1(self.l1(c5)))
        l2 = F.tanh(self.bn2(self.l2(l1)))
        l3 = F.tanh(self.bn3(self.l3(l2)))
        l4 = F.tanh(self.bn4(self.l4(l3)))
        b5 = F.reshape(l4, (l4.shape[0], 256, 1, 1, 1))
        b4 = F.tanh(self.deconv5(b5))
